...
class Prescriptions(db.Model):
	...
	type_prediction=db.Column('type',db.Integer,db.ForeignKey('cancer_types.id'),default=1)
...
def signup(name,password,email,dob,gender):
	...
	format = '%Y/%m/%d'
	user=UserTable(name = name , password = password , email = email , gender=gender , dob=datetime.datetime.strptime(dob,format) , role=3 , user_activated=1 )
